  ...
  async execute(message, args) {
    ...
    if (urlValid) {
      try {
        ...
        videos = await playlist.getVideos(MAX_PLAYLIST_SIZE || 500, { part: "snippet" });
      } catch (error) {
        console.error(error);
    ...
    if (playlistEmbed.description.length >= 1048)
      playlistEmbed.description =
        playlistEmbed.description.substr(0, 1007) + i18n.__("playlist.playlistCharLimit");
